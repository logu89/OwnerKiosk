...
// }}} 
    ...
    public function actionIndex() {
        ...
        switch ($_REQUEST['model']) {
            ...
            case 'sowner': // {{{ 
                $models = sowner::model()->findAll();
                break; // }}} 
            //for userlogin if userlogin is correct then authentication token,userloginid,usertypeid,referenceid are returned
            case 'userlogin':
                // {{{ 
                $models = userlogin::model()->checkUserLogin($_REQUEST['email'], $_REQUEST['pass']);
                break; // }}} 
            //for user register if account with profileid and emailid is not present then it will be added and verification mail will be sent
            case 'userregister':
                // {{{ 
                $models = userlogin::model()->userregister($_REQUEST['name'], $_REQUEST['profileid'], $_REQUEST['emailid'], $_REQUEST['mobile'], $_REQUEST['type']);
                break; // }}} 

            
            //for verifying user who is  registered by verification code and activate his account
            case 'userverify':
                // {{{ 
                $models = userlogin::model()->userverify($_REQUEST['code'], $_REQUEST['emailid']);
                break; // }}} 
...
// }}} 
    ...
    public function actionView() {
        ...
        switch ($_GET['model']) {
            ...
            case 'posts': // {{{ 
                $model = sowner::model()->findByPk($_GET['id']);
                break; // }}} 
            default: // {{{ 
...
// }}} 
    ...
    private function _checkAuth() {
        ...
        // Find the user
        $user = $username; //User::model()->find('LOWER(username)=?', array(strtolower($username)));
        if ($user != "loga") {
            // Error: Unauthorized
